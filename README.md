...
如果你确认操作系统为 Debian/Ubuntu（基于 Linux 5.0 +），并安装了版本合适的 Python 和 GCC，可以运行 `install.sh` 以直接使用推荐配置安装。该脚本完成搭建 Python 环境、编译沙箱两步。运行脚本前可以通过指定环境变量 `PY`，选择 Python 解释器路径。
...
如果你使用的操作系统软件源中没有新版 Python，请考虑 [deadsnakes](https://launchpad.net/~deadsnakes/+archive/ubuntu/ppa) 或 [pyenv](https://github.com/pyenv/pyenv)，后者可能是更好的选择。

使用如下命令创建并激活 venv，然后安装依赖：
...
如果你所在的地区直接访问 PyPI 不稳定，可以使用镜像，例如：
...
沙箱的安全机制基于 resource limits、seccomp-bpf 和 capabilities。
...
- 宏定义模块：根据 `src/macros.cpp`，通过 `$compiler -E -dM` 选项编译出 `build/macros`（如果编译器的选项与这个不同，请修改 `build.py` 相应部分），获取已定义的全部系统调用，存储到 `lib/macros.py`。
...
| :-: | :-: | :-: | :-: | :-: | :-: |
| NOILinux 物理机 | 27 | Ubuntu 20.04.6 | 5.15.0-139-generic | 3.13.9 | 9.4.0 |
| NOILinux + gcc 13 物理机 | 27 | Ubuntu 20.04.6 | 5.15.0-139-generic | 3.13.9 | 13.1.0 |
| Ubuntu 24 虚拟机 | 25 | VMWare Workstation 17.6.4 <br> Ubuntu 24.04.3 | 6.14.0-36-generic | 3.12.3 | 13.3.0 |
| WSL2 | 26 | Windows 11 25H2 (26200.7171) <br> Ubuntu 24.04 | 5.15.167.4-microsoft-standard-WSL2 | 3.12.3 | 13.3.0 |
