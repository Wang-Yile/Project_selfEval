...
from .color import *
from .core import error, warning
...
# 存档管理
_cache_dir = "/tmp/selfeval-persistent-cache"
...
def cache_get(files: list[str], args: list[str], info: str, /, dst: str = None):
    ...
    if not os.path.isfile(path := os.path.join(_cache_dir, sha512)):
        return
    t = max((os.stat(p).st_mtime_ns for p in files), default=0)
    if os.stat(path).st_mtime_ns <= t:
        return
    try:
        with open(path, "rb") as file:
        ...
        error(err)
        return
    fd, path = tempfile.mkstemp(prefix="selfeval-", dir=dst)
    try:
        os.fchmod(fd, mode)
...
def restore(src: str, dst: str, encoding=None, *, strong=True):
    ...
    if os.path.exists(dst):
        try:
            temp_base = tempfile.mkdtemp(prefix="selfeval-")
