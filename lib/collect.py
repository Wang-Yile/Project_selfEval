...
def collect_problem():
    ...
    if _checkers:
        ...
        if len(_checkers) > 1:
            text = f"找到多个校验器，使用 {repr(os.path.relpath(_checkers[0], cwd))}，忽略"
            if len(_checkers) == 2:
                text += " " + repr(os.path.relpath(_checkers[1], cwd))
            else:
                for x in islice(_checkers, 1, None):
                    text += f"\n  - {repr(os.path.relpath(x, cwd))}"
            warning(text, True)
    ...
    if _interactors:
        ...
        if len(_interactors) > 1:
            text = f"找到多个交互库，使用 {repr(os.path.relpath(_interactors[0], cwd))}，忽略"
            if len(_interactors) == 2:
                text += " " + repr(os.path.relpath(_interactors[1], cwd))
            else:
