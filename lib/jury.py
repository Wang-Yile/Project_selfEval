...
def compile_program(cwd: str, source: str, source_backup: str, lang: str, headers: list[str], graders: list[str], /, use_testlib = False, usage = "program"):
    ...
    if not testlib_ok and use_testlib:
        if (p := userconf.UserJudge.testlib) is ModelNULL:
            error("没有找到 testlib，请将 userconf.UserJudge.testlib 设置为你下载的 testlib.h 的绝对路径，或者将 testlib.h 放入数据文件夹中。")
            return
        copy_to(p, wd)
    for file in graders:
        copy_to(file, wd)
    ...
    if typ.startswith("c++"):
        ...
        if is_xok(source):
            if all(os.stat(p).st_mtime_ns <= os.stat(source).st_mtime_ns for p in chain(headers, graders)):
